alpha_values=zeros([1,31,4]);
R_values=[100;500;-100;-500];
for k=1:4
    R=R_values(k);
    j=1;
    for i=5:1:35
        ...
        alpha=mod(rad2deg(trm_state(1))+180,360)-180;
        beta=trm_state(2)*180/pi;
        mu=mod(rad2deg(trm_state(3))+180,360)-180;
        del_e=trm_state(4);
        del_t=trm_state(5);
        del_a=trm_state(6);
        del_r=trm_state(7);
...
figure
subplot(2,1,1)
plot(5:1:35,alpha_values(:,:,1));
hold on
plot(5:1:35,alpha_values(:,:,2));
xlabel('V_a')
ylabel("\alpha")
legend("R=100m","R=500m");
title("\alpha vs V_a")

subplot(2,1,2)
plot(5:1:35,alpha_values(:,:,3),"Color",'magenta');
hold on
plot(5:1:35,alpha_values(:,:,4),'Color','black');
xlabel('V_a')
ylabel("\alpha")
legend("R=-100m","R=-500m");
title("\alpha vs V_a")
