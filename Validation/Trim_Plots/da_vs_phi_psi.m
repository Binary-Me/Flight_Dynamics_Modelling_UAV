...
for dum=1:201
    ...
    phi_j(1,dum)=angles(1,no);
    psi_j(1,dum)=angles(3,no);
end
%%
figure
for i=1:length(vel_j)
    alpha(i)=atan(velocity_b(3,i)/velocity_b(1,i));
end
plot(-1:0.01:1,mod(rad2deg(real(phi_j(1,:)))+180,360)-180);
hold on
plot(-1:0.01:1,mod(rad2deg(real(psi_j(1,:)))+180,360)-180);
plot(-1:0.01:1,mod(rad2deg(real(alpha(1,:)))+180,360)-180);
xlabel('\delta_a')
legend("\phi_s","\psi_s")
title("\phi_s and \psi_s vs \delta_e")
