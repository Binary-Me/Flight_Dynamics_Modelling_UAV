...
%% Inputs
u=25;
%Cordinated Turn - Va=25; R=100; y=0
delta_e=-0.123246256820957;
delta_t=0.337491157476862;
delta_a=0.0312672459416851;
delta_r=-0.0306937254314327;

% %Steady Level Flight u=25
% delta_e=-0.703975;
% delta_t=0;
% delta_a=0;
% delta_r=0;
...
for i=1:no
    ...
    w_actual(:,i)=[p;q;r];
    w_sensor(:,i)= w_actual(:,i)+(0.01.*(2*rand([3,1])-[1;1;1]));
...
% end
% plot(T,mod(rad2deg(alpha_m)+180,360)-180);
...
% end
% plot(T,mod(rad2deg(beta_m)+180,360)-180);
...
h0=[1;0;0];
k0=[0;1;0];
actual_h=zeros([3,length(DCM)/3]);
actual_k=zeros([3,length(DCM)/3]);
sensor_h=zeros([3,length(DCM)/3]);
sensor_k=zeros([3,length(DCM)/3]);
j=1;
    for i=1:length(DCM)/3
        actual_h(:,i)=(DCM(j:j+2,1:3))'*h0;
        actual_k(:,i)=(DCM(j:j+2,1:3))'*k0;
        j=j+3;
    end
for i=1:no
    sensor_h(:,i)=actual_h(:,i)+(0.0001.*(2*rand([3,1])-[1;1;1]));
    sensor_k(:,i)=actual_k(:,i)+(0.0001.*(2*rand([3,1])-[1;1;1]));
end